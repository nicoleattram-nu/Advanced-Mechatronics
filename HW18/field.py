...
Writes current angle to paddle_state.txt for the graph window.
...
def _write_state():
    """Write current angle to file for graph window."""
    try:
        with open('paddle_state.json', 'w') as f:
            json.dump({'angle': smooth_angle}, f)
    except:
        pass
